...
RUN password=`cat /var/jenkins_home/secrets/initialAdminPassword` && java -jar /var/jenkins_home/war/WEB-INF/lib/cli-*.jar -s http://$HOSTNAME:8080/ -auth admin:$password install-plugin kubernetes git
RUN java -jar /usr/share/jenkins/ref/jenkins-cli.jar -s http://$HOSTNAME:8080/ install-plugin  kubernetes:3883.v4d70a_a_a_df034 --deploy --recursive
